...
function showWindowHref(){
    urlS=url.value;
    let arr=urlS.split("?");
    let arguments = arr[1].split("&");
    for(let i=0;i<arguments.length;i++){
        if(arguments[i].indexOf("name")!=-1){
            url_result.value = arguments[i].split("=")[1];
        }
    }
}
...
mul.value = 1;
var currentTimes = 0;
var timeOut = setInterval(timeTest,5000);
function timeTest(){
...
function arrSameStr(){
    ...
    result.value = "The most character is:" + chars[maxI] + " times:" + maxNum;
